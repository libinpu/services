...
DECLARE
  current_role text;
BEGIN
  -- We query profiles directly. We must be careful not to cause recursion if SELECT policies also use is_admin()
  SELECT role INTO current_role FROM public.profiles WHERE id = auth.uid();
  RETURN current_role = 'admin';
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Allow admins to read all profiles (previously they could only read their own if strictly enforced)
...
-- Allow admins to update any provider application
DROP POLICY IF EXISTS "admin_update_provider_applications" ON provider_applications;
CREATE POLICY "admin_update_provider_applications" ON provider_applications FOR UPDATE
  TO authenticated USING (public.is_admin());

-- Allow admins to read all provider applications
DROP POLICY IF EXISTS "admin_select_provider_applications" ON provider_applications;
CREATE POLICY "admin_select_provider_applications" ON provider_applications FOR SELECT
  TO authenticated USING (public.is_admin());
