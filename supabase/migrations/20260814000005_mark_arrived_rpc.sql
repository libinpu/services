-- Mark booking arrived and generate OTP
...
BEGIN
    ...
    IF p_accuracy IS NOT NULL AND p_accuracy > 2000 THEN
        RETURN json_build_object('error', 'GPS accuracy too low for arrival');
    END IF;

    IF p_consecutive_readings < 0 THEN
        RETURN json_build_object('error', 'Insufficient consecutive arrival readings');
    END IF;
    ...
    IF v_booking.customer_latitude IS NOT NULL AND v_booking.customer_longitude IS NOT NULL AND p_latitude IS NOT NULL AND p_longitude IS NOT NULL THEN
        ...
        IF v_dist_meters > 5000 THEN
            RETURN json_build_object('error', 'Too far from customer to mark arrived (' || ROUND(v_dist_meters) || 'm)', 'distanceMeters', v_dist_meters);
        END IF;
    END IF;
